// スタッフ(admin/viewer)の招待・完全削除を行うEdge Function。
...
//
// デプロイはSupabaseダッシュボードの Edge Functions → 「Via Editor」にこのコードを貼る。
// 新規作成すると「Enforce JWT Verification」が既定でオンになっており、ブラウザのCORS
// プリフライト(OPTIONS)ごと弾かれるので必ずオフにすること(この関数は自前でadmin判定している)。
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (role !== 'admin' && role !== 'viewer') return json({ error: '不正なroleです' }, 400)
